    ...
    if ($("#issue_template").val() !== "") {
        ...
        $.ajax({
            ...
            for (var issue_template in template) {
                template.description = (template.description === undefined) ? '' : template.description;
                template.issue_title = (template.issue_title === undefined) ? '' : template.issue_title;
                $("#issue_description").val(oldVal + template.description);
                $("#issue_subject").val(oldSubj + template.issue_title);
                try {
                    if (CKEDITOR.instances.issue_description)
                        CKEDITOR.instances.issue_description.setData(oldVal + template.description);
                } catch (e) {
                    // do nothing.
                }
                if (template.issue_checklist_templates !== undefined) {
                    $('#checklist_form_items').children().remove();
                    $('#issue_description').val(" ");
                    $("#issue_description").val(template.description);
                    //$('#issue_checklist_form').html('');
                    createIssueChecklist(template.issue_checklist_templates);
                }
